// Reuses the EVM design.  Token id is now optional — chips are
// identified by Asset Pubkey on Solana, but we keep the badge.
  ...
  return (
    <div
      ...
      style={{
        ...
        boxShadow: selected
          ? `0 0 20px ${r.color}66, inset 0 0 15px ${r.color}22`
          : `0 0 5px ${r.color}22`,
      }}
    >
      <svg
        ...
        {[0, 90, 180, 270].map((a) => {
          ...
          return (
            <line key={a}
              ...
              stroke={r.color} strokeWidth="3" />
          );
        })}
        <text x="32" y="34" textAnchor="middle" dominantBaseline="central"
          fill={r.color} fontSize="9" fontFamily="monospace" fontWeight="bold">
          {badge}
        </text>
      </svg>
      <div className="font-pixel mt-1 uppercase" style={{ fontSize: 7, color: r.color, letterSpacing: 1 }}>
        {r.name}
      </div>
