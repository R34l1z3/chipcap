...
| Solana programs (devnet) | `treasury wGAqd…ESPp`, **`chip_nft 5opz7a9R…v3SFGT` (SEC-26 — NEW)**, `battle_arena Ae65n…BU8` | chip_nft got a NEW id on the SEC-26 tier deploy (old `A8fqF…k5qQ` orphaned but still deployed — close later to reclaim ~2 SOL; its keypair backed up at `target/deploy/chip_nft-keypair.OLD-*.json`). treasury + battle_arena keypairs unchanged. |
...
- ⏳ **USER dashboard steps to finish (only the user can do — see chat checklist):** Vercel `VITE_CHIP_NFT_PROGRAM=5opz7a9R…` + redeploy; Render `CHIP_NFT_PROGRAM=5opz7a9R…` + Manual Deploy (runs migrate → tier columns). Until both: live site shows the OLD (rarity) chip_nft and new mints won't index. **Pushing main triggers a Vercel rebuild with the new IDL `address` — if the Vercel env var isn't updated first, mint breaks briefly (idl.address=new vs VITE env=old).**
