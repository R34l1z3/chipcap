...
- ⏳ **Remaining (needs user GO — devnet wipe kills in-flight battles):** `gen-program-keys.sh` NEW keypairs + `declare_id!` updates + rebuild → devnet redeploy + `init-programs.js` → `.env` updates everywhere (frontend Vercel `VITE_*_PROGRAM`, indexer Render, relayer) + indexer DB reset (old rows reference dead programs) + Render manual deploy. The old `rarity.*` i18n keys are now dead (kept for parity; prune later).
